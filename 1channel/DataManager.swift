...
//    #pragma mark - helper methods
    ...
    func downloadEpisodesForSeason(seriesName: String, seriesId:String, seasons: NSArray) {
        ...
        for season in seasons {
            ...
            var jsonDict: NSDictionary = NSJSONSerialization.JSONObjectWithData(episodesForSeasonData, options: NSJSONReadingOptions.MutableContainers, error: &error) as NSDictionary
            let results = jsonDict["results"] as NSDictionary
    ...
    func downloadLinksForEpisode(seriesName:String, seriesId:String, season:String, episodeNum:String) {
        ...
        var jsonDict: NSDictionary = NSJSONSerialization.JSONObjectWithData(linksForEpisodeData, options: NSJSONReadingOptions.MutableContainers, error: &error) as NSDictionary
        let results = jsonDict["results"] as NSDictionary
        let links = (results["episode_links"] as NSArray)[0] as NSDictionary
    ...
    func saveObjectToParse(seriesName:String, seriesId: String, episodeInfo: NSDictionary, links: NSDictionary) {
        
        let query = PFQuery(className: seriesName)
        query.whereKey("episodeNumber", equalTo: episodeInfo["episode"])
        query.whereKey("episodeTitle", equalTo: episodeInfo["episodeTitle"])
